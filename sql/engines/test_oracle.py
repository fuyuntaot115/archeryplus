...
# Oracle 驱动（oracledb）为可选依赖：未安装时跳过整个测试类，
# 保证在未启用 Oracle 的环境（含 Windows/macOS/Linux 部署）中测试可正常运行
try:
...
if _HAS_ORACLEDB:
    ...
    # 占位，避免类定义时引用未定义名称（skipIf 会跳过测试执行）
    from sql.engines.models import ResultSet, ReviewResult, ReviewSet

    OracleEngine = object

_ORACLE_SKIP_REASON = (
    "未安装 oracledb 驱动，跳过 Oracle 引擎测试（生产如需连接 Oracle 请安装 oracledb）"
)


@unittest.skipIf(not _HAS_ORACLEDB, _ORACLE_SKIP_REASON)
