...
class TestMssql(TestCase):
    ...
    @patch.object(MssqlEngine, "_table_info")
    def test_capture_delete_rollback(self, mock_table_info):
        mock_cursor = Mock()
        mock_table_info.return_value = ("[dbo].[some_tb]", ["id", "name"], ["id"])
        mock_cursor.description = [("id",), ("name",)]
        mock_cursor.fetchall.return_value = [(1, "before")]

        record = self.engine._capture_rollback(
            mock_cursor, "DELETE FROM some_tb WHERE id = 1"
        )

        self.assertEqual(record[0], "DELETE FROM some_tb WHERE id = 1")
        self.assertIn("INSERT INTO [dbo].[some_tb]", record[1])
        self.assertIn("N'before'", record[1])

    @patch.object(MssqlEngine, "_table_info")
    def test_capture_delete_rollback_without_primary_key(self, mock_table_info):
        mock_cursor = Mock()
        mock_table_info.return_value = ("[dbo].[some_tb]", ["id", "name"], [])
        mock_cursor.description = [("id",), ("name",)]
        mock_cursor.fetchall.return_value = [(1, "before")]
    ...
    @patch.object(MssqlEngine, "_table_info")
    def test_capture_update_rollback(self, mock_table_info):
        mock_cursor = Mock()
        mock_table_info.return_value = ("[dbo].[some_tb]", ["id", "name"], ["id"])
        mock_cursor.description = [("id",), ("name",)]
        mock_cursor.fetchall.return_value = [(1, "before")]
        ...
        record = self.engine._capture_rollback(
            mock_cursor, "UPDATE some_tb SET name = 'after' WHERE id = 1"
        )

        self.assertIn("UPDATE [dbo].[some_tb] SET [name] = N'before'", record[1])
        self.assertIn("WHERE [id] = 1", record[1])

    @patch.object(MssqlEngine, "_table_info")
    def test_capture_insert_rollback(self, mock_table_info):
        mock_cursor = Mock()
        mock_table_info.return_value = ("[dbo].[some_tb]", ["id", "name"], ["id"])
        mock_cursor.description = [("id",), ("name",)]
        ...
        record = self.engine._capture_rollback(
            mock_cursor, "INSERT INTO some_tb (id, name) VALUES (2, 'new')"
        )

        self.assertIn("DELETE FROM [dbo].[some_tb] WHERE [id] = 2", record[1])

    @patch.object(MssqlEngine, "_table_info")
    def test_capture_insert_rollback_without_spaces(self, mock_table_info):
        mock_cursor = Mock()
        mock_table_info.return_value = ("[dbo].[some_tb]", ["id", "name"], ["id"])
        mock_cursor.description = [("id",), ("name",)]
        mock_cursor.fetchall.return_value = []
    ...
    @patch.object(MssqlEngine, "_table_info")
    def test_capture_insert_rollback_without_primary_key(self, mock_table_info):
        mock_cursor = Mock()
        mock_table_info.return_value = ("[dbo].[some_tb]", ["id", "name"], [])
        mock_cursor.description = [("id",), ("name",)]
        mock_cursor.fetchall.return_value = []
        ...
        record = self.engine._capture_rollback(
            mock_cursor, "INSERT INTO some_tb (id, name) VALUES (3, 'new')"
        )

        self.assertIn("[id] = 3 AND [name] = 'new'", record[1])

    def test_capture_procedure_rollback_existing_procedure(self):
        mock_cursor = Mock()
        old_definition = "CREATE PROCEDURE [dbo].[p_test] AS SELECT 1"
        mock_cursor.fetchone.return_value = (old_definition,)

        record = self.engine._capture_procedure_rollback(
            mock_cursor,
            "ALTER PROCEDURE dbo.p_test AS SELECT 2",
        )

        self.assertEqual(record[1], old_definition)
        mock_cursor.execute.assert_called_once()

    def test_capture_procedure_rollback_new_procedure(self):
        mock_cursor = Mock()
        mock_cursor.fetchone.return_value = None

        record = self.engine._capture_procedure_rollback(
            mock_cursor,
            "CREATE OR ALTER PROCEDURE dbo.p_new AS SELECT 1",
        )

        self.assertEqual(record[1], "DROP PROCEDURE [dbo].[p_new];")

    def test_capture_create_procedure_rollback(self):
        mock_cursor = Mock()
        mock_cursor.fetchone.return_value = None

        record = self.engine._capture_procedure_rollback(
            mock_cursor,
            "CREATE PROCEDURE dbo.p_new AS SELECT 1",
        )
