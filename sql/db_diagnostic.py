...
def tablespace(request):
    ...
    if instance.db_type == "mssql":
        query_result = query_engine.tablespace(
            offset, limit, schema_search=schema_search, db_name=db_name
        )
    else:
    ...
    if query_result.error:
        ...
        if instance.db_type == "mssql":
            r = query_engine.tablespace_count(
                schema_search=schema_search, db_name=db_name
            )
        else:
...
def mssql_databases(request):
    ...
    if instance.db_type != "mssql":
        result = {"status": 1, "msg": "仅支持 MSSQL 实例", "data": []}
        return HttpResponse(json.dumps(result), content_type="application/json")

    query_engine = get_engine(instance=instance)
    query_result = query_engine.get_all_databases()
    if query_result.error:
        result = {"status": 1, "msg": query_result.error, "data": []}
    else:
        result = {"status": 0, "msg": "ok", "data": query_result.rows}
