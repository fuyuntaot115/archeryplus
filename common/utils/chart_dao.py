...
class ChartDao(object):
    ...
    @staticmethod
    def __query(sql, ignore_missing_table=False):
        cursor = connection.cursor()
        try:
            cursor.execute(sql)
        except Exception as e:
            # 慢查询历史表（mysql_slow_query_review_history 等）为可选功能，
            # 若未导入初始化 SQL 导致表不存在（MySQL 错误码 1146），
            # 返回空数据而非抛异常，保证 Dashboard 等页面可正常展示
            if ignore_missing_table and e.args and e.args[0] == 1146:
...
group by date(date_add(ts_min, interval 8 HOUR));"""
        return self.__query(sql, ignore_missing_table=True)
